...
% number of steps.
mu1        = zeros(6,1); % Let us say a constant wind is blowing in one
                         % direction, say horizontal.
                         % This will create some negative angular
                         % perturbance and positive Hz perturbance.
mu1(1)      = -0.2;
mu1(4)      = -0.3;
mu1(2)      = -0.2;
mu1(5)      = -0.5;
...
% Nominal States and Inputs for Hovering:
xhov = [0,10,5,0,0,0]'; % Hovering position.
xnom = repmat(xhov,1,N); % Time-series for hovering equilibrium states.
uhov = [g/2 g/2 g]'; % hovering inputs.
unom = repmat(uhov,1,N); % Time-series for hovering equilibrium inputs.
...
q = (100)*eye(n,n); % Cost associated with state deviation;
...
p =(1000000)*eye(n,n); % Final/Terminal state cost.
